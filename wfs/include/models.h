...
class ChunkReplicasSlaveModel : public DBEntity<ChunkReplicasSlaveModel>
{
public:
    // Member variables
    size_t id;
    ...
    size_t file_id;
    std::string file_path;
    std::string data_hash; // Store the hash as a string (for easier manipulation)
    std::string created_at;

    // Constructors
    ChunkReplicasSlaveModel();
    ChunkReplicasSlaveModel(size_t id, size_t replica_id, size_t chunk_id,
    ...
    ChunkReplicasSlaveModel mapRowToEntity(const MYSQL_ROW &row, [[maybe_unused]] MYSQL_FIELD *fields) const;

    // Insert a new record into the chunk_replicas_slave table
    bool insertRecord(MYSQL *conn);
    bool updateHash(MYSQL *conn);
};
